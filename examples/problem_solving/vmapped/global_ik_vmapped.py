...
opt_state = opt.init(cur_q)

loss_grad_vmap = jax.jit(jax.vmap(loss_grad, in_axes=(None, None, 0)))

print("Compiling")
t0 = perf_counter()
updates, opt_state = global_ik_step(
    model=mjx_model, tasks=tasks, q=cur_q, optimizer=opt, state=opt_state, loss_grad=loss_grad_vmap
)
